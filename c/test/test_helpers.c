/*
 * test_helpers.c - Test helper functions in MPC designation converter
 *
...
/* Test mpc_to_report_format */
static void test_to_report(const char *input, const char *expected, const char *desc) {
    (void)desc;
    char output[16];
    int err = mpc_to_report_format(input, output, sizeof(output));

    if (err == MPC_OK && strcmp(output, expected) == 0) {
        printf("  PASS: to_report(\"%s\") -> \"%s\"\n", input, output);
        passed++;
    } else if (err != MPC_OK) {
        printf("  FAIL: to_report(\"%s\"): error %d (%s)\n", input, err, mpc_strerror(err));
        failed++;
    } else {
        printf("  FAIL: to_report(\"%s\"): expected \"%s\", got \"%s\"\n", input, expected, output);
        failed++;
    }
}

/* Test mpc_from_report_format */
static void test_from_report(const char *input, const char *expected, const char *desc) {
    (void)desc;
    char output[16];
    int err = mpc_from_report_format(input, output, sizeof(output));

    if (err == MPC_OK && strcmp(output, expected) == 0) {
        printf("  PASS: from_report(\"%s\") -> \"%s\"\n", input, output);
        passed++;
    } else if (err != MPC_OK) {
        printf("  FAIL: from_report(\"%s\"): error %d (%s)\n", input, err, mpc_strerror(err));
        failed++;
    } else {
        printf("  FAIL: from_report(\"%s\"): expected \"%s\", got \"%s\"\n", input, expected, output);
        failed++;
    }
}

/* Test mpc_has_fragment */
static void test_has_fragment(const char *input, int expected, const char *desc) {
    (void)desc;
    int result = mpc_has_fragment(input);

    if (result == expected) {
        printf("  PASS: has_fragment(\"%s\") -> %d\n", input, result);
        passed++;
    } else {
        printf("  FAIL: has_fragment(\"%s\"): expected %d, got %d\n", input, expected, result);
        failed++;
    }
}

/* Test mpc_get_fragment */
static void test_get_fragment(const char *input, const char *expected, const char *desc) {
    (void)desc;
    char output[8];
    int err = mpc_get_fragment(input, output, sizeof(output));

    if (err == MPC_OK && strcmp(output, expected) == 0) {
        printf("  PASS: get_fragment(\"%s\") -> \"%s\"\n", input, output);
        passed++;
    } else if (err != MPC_OK) {
        printf("  FAIL: get_fragment(\"%s\"): error %d (%s)\n", input, err, mpc_strerror(err));
        failed++;
    } else {
        printf("  FAIL: get_fragment(\"%s\"): expected \"%s\", got \"%s\"\n", input, expected, output);
        failed++;
    }
}

/* Test mpc_get_parent */
static void test_get_parent(const char *input, const char *expected, const char *desc) {
    (void)desc;
    char output[32];
    int err = mpc_get_parent(input, output, sizeof(output));

    if (err == MPC_OK && strcmp(output, expected) == 0) {
        printf("  PASS: get_parent(\"%s\") -> \"%s\"\n", input, output);
        passed++;
    } else if (err != MPC_OK) {
        printf("  FAIL: get_parent(\"%s\"): error %d (%s)\n", input, err, mpc_strerror(err));
        failed++;
    } else {
        printf("  FAIL: get_parent(\"%s\"): expected \"%s\", got \"%s\"\n", input, expected, output);
        failed++;
    }
}

/* Test mpc_designations_equal */
static void test_equal(const char *d1, const char *d2, int expected, const char *desc) {
    (void)desc;
    int result = mpc_designations_equal(d1, d2);

    if (result == expected) {
        printf("  PASS: equal(\"%s\", \"%s\") -> %d\n", d1, d2, result);
        passed++;
    } else {
        printf("  FAIL: equal(\"%s\", \"%s\"): expected %d, got %d\n", d1, d2, expected, result);
        failed++;
    }
}

int main(void) {
    printf("=== MPC Designation Helper Function Tests ===\n\n");

    /* Test mpc_to_report_format */
    printf("--- mpc_to_report_format ---\n");
