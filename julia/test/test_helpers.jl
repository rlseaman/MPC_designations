...
function test_to_report(input, expected, desc)
    global passed, failed
    try
        output = to_report_format(input)
        if output == expected
            println("  PASS: to_report(\"$input\") -> \"$output\"")
            passed += 1
        else
            println("  FAIL: to_report(\"$input\"): expected \"$expected\", got \"$output\"")
            failed += 1
        end
    catch e
        println("  FAIL: to_report(\"$input\"): error $e")
        failed += 1
    end
end

function test_from_report(input, expected, desc)
    global passed, failed
    try
        output = from_report_format(input)
        if output == expected
            println("  PASS: from_report(\"$input\") -> \"$output\"")
            passed += 1
        else
            println("  FAIL: from_report(\"$input\"): expected \"$expected\", got \"$output\"")
            failed += 1
        end
    catch e
        println("  FAIL: from_report(\"$input\"): error $e")
        failed += 1
    end
end

function test_has_fragment(input, expected, desc)
    global passed, failed
    result = has_fragment(input)
    if result == expected
        println("  PASS: has_fragment(\"$input\") -> $result")
        passed += 1
    else
        println("  FAIL: has_fragment(\"$input\"): expected $expected, got $result")
        failed += 1
    end
end

function test_get_fragment(input, expected, desc)
    global passed, failed
    output = get_fragment(input)
    if output == expected
        println("  PASS: get_fragment(\"$input\") -> \"$output\"")
        passed += 1
    else
        println("  FAIL: get_fragment(\"$input\"): expected \"$expected\", got \"$output\"")
        failed += 1
    end
end

function test_get_parent(input, expected, desc)
    global passed, failed
    output = get_parent(input)
    if output == expected
        println("  PASS: get_parent(\"$input\") -> \"$output\"")
        passed += 1
    else
        println("  FAIL: get_parent(\"$input\"): expected \"$expected\", got \"$output\"")
        failed += 1
    end
end
