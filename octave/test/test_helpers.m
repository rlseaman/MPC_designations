% test_helpers.m - Test helper functions in MPC designation converter
%
...
function test_to_report(input_val, expected, desc)
  global passed failed;
  try
    result = mpc_to_report_format(input_val);
    if strcmp(result, expected)
      printf('  PASS: to_report_format("%s") -> "%s"\n', input_val, result);
      passed = passed + 1;
    else
      printf('  FAIL: to_report_format("%s"): expected "%s", got "%s"\n', input_val, expected, result);
      failed = failed + 1;
    end
  catch err
    printf('  FAIL: to_report_format("%s"): error - %s\n', input_val, err.message);
    failed = failed + 1;
  end
end

function test_from_report(input_val, expected, desc)
  global passed failed;
  try
    result = mpc_from_report_format(input_val);
    if strcmp(result, expected)
      printf('  PASS: from_report_format("%s") -> "%s"\n', input_val, result);
      passed = passed + 1;
    else
      printf('  FAIL: from_report_format("%s"): expected "%s", got "%s"\n', input_val, expected, result);
      failed = failed + 1;
    end
  catch err
    printf('  FAIL: from_report_format("%s"): error - %s\n', input_val, err.message);
    failed = failed + 1;
  end
end

function test_has_fragment(input_val, expected, desc)
  global passed failed;
  result = mpc_has_fragment(input_val);
  if result == expected
...
function test_get_fragment(input_val, expected, desc)
  global passed failed;
  try
    result = mpc_get_fragment(input_val);
    if strcmp(result, expected)
    ...
    printf('  FAIL: get_fragment("%s"): error - %s\n', input_val, err.message);
    failed = failed + 1;
  end
end

function test_get_parent(input_val, expected, desc)
  global passed failed;
  try
    result = mpc_get_parent(input_val);
    if strcmp(result, expected)
    ...
    printf('  FAIL: get_parent("%s"): error - %s\n', input_val, err.message);
    failed = failed + 1;
  end
end

function test_equal(d1, d2, expected, desc)
  global passed failed;
  result = mpc_designations_equal(d1, d2);
  if result == expected
    printf('  PASS: designations_equal("%s", "%s") -> %d\n', d1, d2, result);
    passed = passed + 1;
  else
    printf('  FAIL: designations_equal("%s", "%s"): expected %d, got %d\n', d1, d2, expected, result);
    failed = failed + 1;
  end
end

printf('=== MPC Designation Helper Function Tests (Octave) ===\n\n');

% Test to_report_format
printf('--- to_report_format ---\n');
