## MPC Designation Helper Function Tests - Nim Implementation
...
proc testToReport(input, expected, desc: string) =
  let result = toReportFormat(input)
  if result == expected:
...
proc testFromReport(input, expected, desc: string) =
  let result = fromReportFormat(input)
  if result == expected:
...
proc testHasFragment(input: string, expected: bool, desc: string) =
  let result = hasFragment(input)
  if result == expected:
    echo "  PASS: hasFragment(\"", input, "\") -> ", result
    inc passed
  else:
    echo "  FAIL: hasFragment(\"", input, "\"): expected ", expected, ", got ", result
    inc failed

proc testGetFragment(input, expected, desc: string) =
  let result = getFragment(input)
  if result == expected:
...
proc testGetParent(input, expected, desc: string) =
  let result = getParent(input)
  if result == expected:
...
proc testEqual(d1, d2: string, expected: bool, desc: string) =
  let result = designationsEqual(d1, d2)
  if result == expected:
    echo "  PASS: designationsEqual(\"", d1, "\", \"", d2, "\") -> ", result
    inc passed
  else:
    echo "  FAIL: designationsEqual(\"", d1, "\", \"", d2, "\"): expected ", expected, ", got ", result
    inc failed

echo "=== MPC Designation Helper Function Tests (Nim) ==="
echo ""
