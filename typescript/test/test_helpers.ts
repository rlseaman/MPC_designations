...
import {
    ...
    getParent,
    designationsEqual
} from '../src/mpc_designation';

let passed = 0;
let failed = 0;

function testToReport(input: string, expected: string, _desc: string): void {
    try {
        const result = toReportFormat(input);
        if (result === expected) {
            console.log(`  PASS: toReportFormat("${input}") -> "${result}"`);
            passed++;
        } else {
            console.log(`  FAIL: toReportFormat("${input}"): expected "${expected}", got "${result}"`);
            failed++;
        }
    } catch (e: any) {
        console.log(`  FAIL: toReportFormat("${input}"): error - ${e.message}`);
        failed++;
    }
}

function testFromReport(input: string, expected: string, _desc: string): void {
    try {
        const result = fromReportFormat(input);
        if (result === expected) {
            console.log(`  PASS: fromReportFormat("${input}") -> "${result}"`);
            passed++;
        } else {
            console.log(`  FAIL: fromReportFormat("${input}"): expected "${expected}", got "${result}"`);
            failed++;
        }
    } catch (e: any) {
        console.log(`  FAIL: fromReportFormat("${input}"): error - ${e.message}`);
        failed++;
    }
}

function testHasFragment(input: string, expected: boolean, _desc: string): void {
    const result = hasFragment(input);
    if (result === expected) {
        console.log(`  PASS: hasFragment("${input}") -> ${result}`);
        passed++;
    } else {
        console.log(`  FAIL: hasFragment("${input}"): expected ${expected}, got ${result}`);
        failed++;
    }
}

function testGetFragment(input: string, expected: string, _desc: string): void {
    try {
        const result = getFragment(input);
        if (result === expected) {
            console.log(`  PASS: getFragment("${input}") -> "${result}"`);
            passed++;
        } else {
            console.log(`  FAIL: getFragment("${input}"): expected "${expected}", got "${result}"`);
            failed++;
        }
    } catch (e: any) {
        console.log(`  FAIL: getFragment("${input}"): error - ${e.message}`);
        failed++;
    }
}

function testGetParent(input: string, expected: string, _desc: string): void {
    try {
        const result = getParent(input);
        if (result === expected) {
            console.log(`  PASS: getParent("${input}") -> "${result}"`);
            passed++;
        } else {
            console.log(`  FAIL: getParent("${input}"): expected "${expected}", got "${result}"`);
            failed++;
        }
    } catch (e: any) {
        console.log(`  FAIL: getParent("${input}"): error - ${e.message}`);
        failed++;
    }
}

function testEqual(d1: string, d2: string, expected: boolean, _desc: string): void {
    const result = designationsEqual(d1, d2);
    if (result === expected) {
        console.log(`  PASS: designationsEqual("${d1}", "${d2}") -> ${result}`);
        passed++;
    } else {
        console.log(`  FAIL: designationsEqual("${d1}", "${d2}"): expected ${expected}, got ${result}`);
        failed++;
    }
}

console.log('=== MPC Designation Helper Function Tests (TypeScript) ===');
console.log();

// Test toReportFormat
console.log('--- toReportFormat ---');
