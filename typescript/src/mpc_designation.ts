/**
 * mpc_designation.ts - Convert between packed and unpacked MPC designations
 *
...
const CENTURY_CODES: Record<string, number> = {
    'A': 10, 'B': 11, 'C': 12, 'D': 13, 'E': 14, 'F': 15,
    'G': 16, 'H': 17, 'I': 18, 'J': 19, 'K': 20, 'L': 21
};

const REVERSE_CENTURY_CODES: Record<number, string> = {};
for (const [k, v] of Object.entries(CENTURY_CODES)) {
    REVERSE_CENTURY_CODES[v] = k;
}

// Survey codes mapping
const SURVEY_PACKED_TO_UNPACKED: Record<string, string> = {
    'PLS': 'P-L',
    'T1S': 'T-1',
    'T2S': 'T-2',
    'T3S': 'T-3'
...
const SURVEY_UNPACKED_TO_PACKED: Record<string, string> = {};
...
const SATELLITE_PLANET_NAMES: Record<string, string> = {
    'J': 'Jupiter',
    'S': 'Saturn',
    'U': 'Uranus',
    'N': 'Neptune'
...
export const MAX_ASTEROID_NUMBER = 15396335;

export const VERSION = '1.0.0';

/**
 * Format type for designations
 */
export type FormatType = 'packed' | 'unpacked';

/**
 * Designation type categories
 */
 ...
 */
export class MPCDesignationError extends Error {
 ...
 */
function validateRawInput(s: string): void {
 ...
 */
function validateWhitespace(s: string): void {
    let prevSpace = false;
    for (let i = 0; i < s.length; i++) {
        const code = s.charCodeAt(i);
        if (code < 32 || code > 126) {
 ...
 */
function isValidHalfMonth(letter: string): boolean {
 ...
 */
function base62ToNum(char: string): number {
 ...
 */
function base62StringToNum(s: string): number {
    let result = 0;
    for (let i = 0; i < s.length; i++) {
 ...
 */
function numToBase62String(num: number, width: number = 4): string {
    const result: string[] = [];
 ...
 */
export function unpackPermanent(packed: string): number {
    packed = packed.trim();
 ...
 */
export function packPermanent(number: number): string {
 ...
 */
function decodeCycleCount(encoded: string): number {
 ...
 */
function encodeCycleCount(count: number): string {
 ...
 */
function letterToPosition(letter: string): number {
 ...
 */
function positionToLetter(pos: number): string {
    if (pos < 1 || pos > 25) {
 ...
 */
function needsExtendedFormat(cycle: number): boolean {
    return cycle >= 620;
}
 ...
 */
function packExtendedProvisional(year: number, halfMonth: string, secondLetter: string, cycle: number): string {
 ...
 */
export function unpackExtendedProvisional(packed: string): string {
    packed = packed.trim();
 ...
 */
export function unpackProvisional(packed: string): string {
    packed = packed.trim();
 ...
 */
export function packProvisional(unpacked: string): string {
 ...
 */
export function unpackCometProvisional(packed: string): string {
    packed = packed.trim();
    const length = packed.length;
 ...
 */
export function packCometProvisional(unpacked: string): string {
    unpacked = unpacked.trim();
 ...
 */
export function unpackCometNumbered(packed: string): string {
    packed = packed.trim();
 ...
 */
export function packCometNumbered(unpacked: string): string {
    unpacked = unpacked.trim();
 ...
 */
export function unpackSatellite(packed: string): string {
    packed = packed.trim();
 ...
 */
export function packSatellite(unpacked: string): string {
    unpacked = unpacked.trim();
 ...
 */
function encodeBceYear(year: number): [string, string] {
    if (year >= 0) {
 ...
 */
function decodeBceYear(prefix: string, code: string): number {
 ...
 */
function isAncientYear(year: number): boolean {
    return year < 1000;
}
 ...
 */
function packAncientCometProvisional(cometType: string, year: number, halfMonth: string, orderNum: number, fragment: string = ''): string {
    const orderEncoded = encodeCycleCount(orderNum);
    const fragmentCode = fragment ? fragment.toLowerCase() : '0';
 ...
 */
export function unpackAncientCometProvisional(packed: string): string {
    packed = packed.trim();
 ...
 */
function isAsteroidStylePacked(provisionalPart: string): boolean {
 ...
 */
function isAsteroidStyleUnpacked(provisional: string): boolean {
 ...
 */
export function unpackCometFull(packed: string): string {
    let length = packed.length;

    let cometType: string;
    let provisionalPart: string;
    let numStr: string;
 ...
 */
export function packCometFull(unpacked: string): string {
    unpacked = unpacked.trim();
 ...
 */
export function detectFormat(designation: string): FormatInfo {
    ...
    if (des.length === 8 && des[0] === 'S') {
        if (/^S[A-L][0-9]{2}[JSUN][0-9A-Za-z]{2}0$/.test(des)) {
            ...
            return {
                format: 'packed',
                type: 'satellite',
                subtype: `natural satellite (${planetName})`
            };
        }
    }
    ...
    if (des.length === 6 || des.length === 7) {
        if (/^[0-9]{4}[PD][a-z]{1,2}$/.test(des)) {
            ...
            return {
                format: 'packed',
                type: 'comet_numbered',
                subtype: fragLen === 1 ? 'comet numbered with fragment' : 'comet numbered with 2-letter fragment'
            };
        }
    }
    ...
    if (des.length === 7) {
        if (/^[IJKL][0-9]{2}[A-Z][0-9A-Za-z]{2}[0-9a-z]$/.test(des)) {
            return {
                format: 'packed',
                type: 'comet_provisional',
                subtype: 'comet provisional'
            };
        }
    }
    ...
    if (match) {
        ...
        let subtype: string;
        if (fragment) {
            subtype = fragment.length === 1 ? 'comet numbered with fragment' : 'comet numbered with 2-letter fragment';
        } else {
            subtype = 'comet numbered';
        }
        return {
 ...
 */
export function convert(designation: string): ConversionResult {
 ...
 */
export function convertSimple(designation: string): string {
 ...
 */
export function pack(designation: string): string {
    const info = detectFormat(designation);
 ...
 */
export function unpack(designation: string): string {
    const info = detectFormat(designation);
 ...
 */
export function isValidDesignation(designation: string | null | undefined): boolean {
 ...
 */
export function toReportFormat(minimal: string): string {
 ...
 */
export function fromReportFormat(report: string): string {
    if (report.length > 12) {
 ...
 */
export function hasFragment(desig: string): boolean {
    let info: FormatInfo;
    try {
 ...
 */
export function getFragment(desig: string): string {
 ...
 */
export function getParent(desig: string): string {
    const info = detectFormat(desig);
    const dtype = info.type;
 ...
 */
export function designationsEqual(desig1: string, desig2: string): boolean {
    try {
