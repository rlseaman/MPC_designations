/**
 * test_helpers.swift - Test helper functions in MPC designation converter
 *
...
// Test toReportFormat
func testToReport(_ input: String, _ expected: String, _ desc: String) {
    do {
        let output = try toReportFormat(input)
        if output == expected {
            print("  PASS: toReport(\"\(input)\") -> \"\(output)\"")
            passed += 1
        } else {
            print("  FAIL: toReport(\"\(input)\"): expected \"\(expected)\", got \"\(output)\"")
            failed += 1
        }
    } catch {
        print("  FAIL: toReport(\"\(input)\"): error \(error)")
        failed += 1
    }
}

// Test fromReportFormat
func testFromReport(_ input: String, _ expected: String, _ desc: String) {
    do {
        let output = try fromReportFormat(input)
        if output == expected {
            print("  PASS: fromReport(\"\(input)\") -> \"\(output)\"")
            passed += 1
        } else {
            print("  FAIL: fromReport(\"\(input)\"): expected \"\(expected)\", got \"\(output)\"")
            failed += 1
        }
    } catch {
        print("  FAIL: fromReport(\"\(input)\"): error \(error)")
        failed += 1
    }
}

// Test hasFragment
func testHasFragment(_ input: String, _ expected: Bool, _ desc: String) {
    let result = hasFragment(input)
    if result == expected {
        print("  PASS: hasFragment(\"\(input)\") -> \(result)")
        passed += 1
    } else {
        print("  FAIL: hasFragment(\"\(input)\"): expected \(expected), got \(result)")
        failed += 1
    }
}

// Test getFragment
func testGetFragment(_ input: String, _ expected: String, _ desc: String) {
    let output = getFragment(input)
    if output == expected {
        print("  PASS: getFragment(\"\(input)\") -> \"\(output)\"")
        passed += 1
    } else {
        print("  FAIL: getFragment(\"\(input)\"): expected \"\(expected)\", got \"\(output)\"")
        failed += 1
    }
}

// Test getParent
func testGetParent(_ input: String, _ expected: String, _ desc: String) {
    let output = getParent(input)
    if output == expected {
        print("  PASS: getParent(\"\(input)\") -> \"\(output)\"")
        passed += 1
    } else {
        print("  FAIL: getParent(\"\(input)\"): expected \"\(expected)\", got \"\(output)\"")
        failed += 1
    }
}

// Test designationsEqual
func testEqual(_ d1: String, _ d2: String, _ expected: Bool, _ desc: String) {
    let result = designationsEqual(d1, d2)
    if result == expected {
        print("  PASS: equal(\"\(d1)\", \"\(d2)\") -> \(result)")
        passed += 1
    } else {
        print("  FAIL: equal(\"\(d1)\", \"\(d2)\"): expected \(expected), got \(result)")
        failed += 1
    }
}

func runHelperTests() {
    print("=== MPC Designation Helper Function Tests ===\n")

    // Test toReportFormat
    print("--- toReportFormat ---")
