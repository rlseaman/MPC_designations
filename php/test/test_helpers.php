...
function test_to_report(string $input, string $expected, string $desc): void {
    global $passed, $failed;
    try {
        $result = MPCDesignation::toReportFormat($input);
        if ($result === $expected) {
            echo "  PASS: toReportFormat(\"$input\") -> \"$result\"\n";
            $passed++;
        } else {
            echo "  FAIL: toReportFormat(\"$input\"): expected \"$expected\", got \"$result\"\n";
            $failed++;
        }
    } catch (Exception $e) {
        echo "  FAIL: toReportFormat(\"$input\"): error - " . $e->getMessage() . "\n";
        $failed++;
    }
}

function test_from_report(string $input, string $expected, string $desc): void {
    global $passed, $failed;
    try {
        $result = MPCDesignation::fromReportFormat($input);
        if ($result === $expected) {
            echo "  PASS: fromReportFormat(\"$input\") -> \"$result\"\n";
            $passed++;
        } else {
            echo "  FAIL: fromReportFormat(\"$input\"): expected \"$expected\", got \"$result\"\n";
            $failed++;
        }
    } catch (Exception $e) {
        echo "  FAIL: fromReportFormat(\"$input\"): error - " . $e->getMessage() . "\n";
        $failed++;
    }
}

function test_has_fragment(string $input, bool $expected, string $desc): void {
    global $passed, $failed;
    $result = MPCDesignation::hasFragment($input);
    if ($result === $expected) {
        $resultStr = $result ? 'true' : 'false';
        echo "  PASS: hasFragment(\"$input\") -> $resultStr\n";
        $passed++;
    } else {
        $expectedStr = $expected ? 'true' : 'false';
        $resultStr = $result ? 'true' : 'false';
        echo "  FAIL: hasFragment(\"$input\"): expected $expectedStr, got $resultStr\n";
        $failed++;
    }
}

function test_get_fragment(string $input, string $expected, string $desc): void {
    global $passed, $failed;
    try {
        $result = MPCDesignation::getFragment($input);
        if ($result === $expected) {
            echo "  PASS: getFragment(\"$input\") -> \"$result\"\n";
            $passed++;
        } else {
            echo "  FAIL: getFragment(\"$input\"): expected \"$expected\", got \"$result\"\n";
            $failed++;
        }
    } catch (Exception $e) {
        echo "  FAIL: getFragment(\"$input\"): error - " . $e->getMessage() . "\n";
        $failed++;
    }
}

function test_get_parent(string $input, string $expected, string $desc): void {
    global $passed, $failed;
    try {
        $result = MPCDesignation::getParent($input);
        if ($result === $expected) {
            echo "  PASS: getParent(\"$input\") -> \"$result\"\n";
            $passed++;
        } else {
            echo "  FAIL: getParent(\"$input\"): expected \"$expected\", got \"$result\"\n";
            $failed++;
        }
    } catch (Exception $e) {
        echo "  FAIL: getParent(\"$input\"): error - " . $e->getMessage() . "\n";
        $failed++;
    }
}

function test_equal(string $d1, string $d2, bool $expected, string $desc): void {
    global $passed, $failed;
    $result = MPCDesignation::designationsEqual($d1, $d2);
    if ($result === $expected) {
        $resultStr = $result ? 'true' : 'false';
        echo "  PASS: designationsEqual(\"$d1\", \"$d2\") -> $resultStr\n";
        $passed++;
    } else {
        $expectedStr = $expected ? 'true' : 'false';
        $resultStr = $result ? 'true' : 'false';
        echo "  FAIL: designationsEqual(\"$d1\", \"$d2\"): expected $expectedStr, got $resultStr\n";
        $failed++;
    }
}

echo "=== MPC Designation Helper Function Tests (PHP) ===\n\n";
