#!/usr/bin/env tclsh
#
# mpc_designation.tcl - Convert between packed and unpacked asteroid/comet designations
#
# Based on Minor Planet Center specifications:
# https://www.minorplanetcenter.net/iau/info/PackedDes.html
#
# Usage: mpc_designation.tcl [-v|--verbose] <designation> [designation ...]
#
# The program auto-detects whether the input is packed or unpacked
# and converts to the other format. Supports both asteroids and comets.
#
# Options:
...
namespace eval MPCDesignation {
    ...
    proc packProvisional {unpacked} {
        ...
        if {[regexp {^(\d+)\s+(P-L|T-1|T-2|T-3)$} $unpacked -> number survey]} {
        ...
        if {[regexp {^[AB](\d)(\d{2})\s+([A-Z])([A-Z])$} $unpacked -> centuryDigit yearShort halfMonth secondLetter]} {
        ...
        if {![regexp {^(\d{4})\s+([A-Z])([A-Z])(\d*)$} $unpacked -> year halfMonth secondLetter cycleStr]} {
    ...
    proc packCometProvisional {unpacked} {
        ...
        if {![regexp {^(\d{4})\s+([A-Z])(\d+)(?:-([A-Z]{1,2}))?$} $unpacked -> year halfMonth orderNum fragment]} {
    ...
    proc packSatellite {unpacked} {
        ...
        if {![regexp {^S/(\d{4})\s+([JSUN])\s+(\d+)$} $unpacked -> year planet number]} {
    ...
    proc isAsteroidStyleUnpacked {provisional} {
        ...
        if {[regexp {^\d{4}\s+[A-Z][A-Z]} $provisional]} {
            return 1
        }
        return 0
    }
    ...
    proc packCometFull {unpacked} {
        ...
        if {![regexp {^(\d*)([PCDXAI])/(-?\d+)\s+(.+)$} $unpacked -> number cometType year provPart]} {
    ...
    proc detectFormat {designation} {
        ...
        if {[regexp {^(\d+)\s+P-L$} $des]} {
            dict set result format unpacked
            dict set result type survey
            dict set result subtype "survey (Palomar-Leiden)"
            return $result
        }
        if {[regexp {^(\d+)\s+T-([123])$} $des -> num surveyNum]} {
            dict set result format unpacked
            dict set result type survey
            dict set result subtype "survey (Trojan T-$surveyNum)"
            return $result
        ...
        if {[regexp {^[AB]\d{3}\s+[A-Z][A-Z]$} $des]} {
            dict set result format unpacked
            dict set result type provisional
        ...
        if {[regexp {^S/(\d{4})\s+([JSUN])\s+(\d+)$} $des -> year planet num]} {
            variable satellitePlanetNames
            dict set result format unpacked
            dict set result type satellite
            dict set result subtype "natural satellite ([dict get $satellitePlanetNames $planet])"
        ...
        if {[regexp {^(\d*)([PCDXAI])/(-?\d+)\s+([A-Z][A-Z0-9]+)(?:-([A-Z]{1,2}))?$} $des -> num ctype year provPart fragment]} {
            variable cometTypeDescriptions
            dict set result format unpacked
            dict set result type comet_full
...
#
proc printUsage {} {
    puts stderr "Usage: mpc_designation.tcl \[-v|--verbose\] <designation> \[designation ...\]"
    puts stderr ""
    ...
    puts stderr "    mpc_designation.tcl 03202      -> 3202"
    puts stderr "    mpc_designation.tcl 3202       -> 03202"
    puts stderr "    mpc_designation.tcl A0345      -> 100345"
    puts stderr "    mpc_designation.tcl 100345     -> A0345"
    puts stderr "    mpc_designation.tcl '~0000'    -> 620000"
    puts stderr "    mpc_designation.tcl 620000     -> ~0000"
    puts stderr ""
    puts stderr "  Provisional designations:"
    puts stderr "    mpc_designation.tcl J95X00A    -> 1995 XA"
    puts stderr "    mpc_designation.tcl '1995 XA'  -> J95X00A"
    puts stderr "    mpc_designation.tcl J98SA8Q    -> 1998 SQ108"
    puts stderr "    mpc_designation.tcl '1998 SQ108' -> J98SA8Q"
    puts stderr ""
    puts stderr "  Survey designations:"
    puts stderr "    mpc_designation.tcl PLS2040    -> 2040 P-L"
    puts stderr "    mpc_designation.tcl '2040 P-L' -> PLS2040"
    puts stderr ""
    puts stderr "COMET Examples:"
    puts stderr "  Numbered periodic comets:"
    puts stderr "    mpc_designation.tcl 0001P      -> 1P"
    puts stderr "    mpc_designation.tcl 1P         -> 0001P"
    puts stderr "    mpc_designation.tcl 0354P      -> 354P"
    puts stderr ""
    puts stderr "  Provisional comets (type/year letter number):"
    puts stderr "    mpc_designation.tcl 'C/1995 O1'    -> \"    CJ95O010\" (12-char packed)"
    puts stderr "    mpc_designation.tcl 'P/2019 A4'    -> \"    PK19A040\""
    puts stderr "    mpc_designation.tcl 'D/1993 F2-B'  -> \"    DJ93F02b\" (fragment B)"
    puts stderr ""
    puts stderr "  Packed comet provisional (7-char):"
    puts stderr "    mpc_designation.tcl J95O010    -> 1995 O1"
    puts stderr "    mpc_designation.tcl J94P01b    -> 1994 P1-B"
    puts stderr ""
    puts stderr "  Full packed comet (12-char with number):"
    puts stderr "    mpc_designation.tcl '0001PJ82U010' -> 1P/1982 U1"
    puts stderr ""
    ...
    puts stderr "    mpc_designation.tcl 03202 J95X00A 'C/1995 O1' 1P"
    puts stderr ""
    puts stderr "Verbose output:"
    puts stderr "    mpc_designation.tcl -v 'C/1995 O1'"
}
