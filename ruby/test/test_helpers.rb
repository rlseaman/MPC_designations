...
def test_to_report(input, expected, desc)
  begin
    result = MPCDesignation.to_report_format(input)
    if result == expected
      puts "  PASS: to_report_format(\"#{input}\") -> \"#{result}\""
      $passed += 1
    else
      puts "  FAIL: to_report_format(\"#{input}\"): expected \"#{expected}\", got \"#{result}\""
      $failed += 1
    end
  rescue StandardError => e
    puts "  FAIL: to_report_format(\"#{input}\"): error - #{e.message}"
    $failed += 1
  end
end

def test_from_report(input, expected, desc)
  begin
    result = MPCDesignation.from_report_format(input)
    if result == expected
      puts "  PASS: from_report_format(\"#{input}\") -> \"#{result}\""
      $passed += 1
    else
      puts "  FAIL: from_report_format(\"#{input}\"): expected \"#{expected}\", got \"#{result}\""
      $failed += 1
    end
  rescue StandardError => e
    puts "  FAIL: from_report_format(\"#{input}\"): error - #{e.message}"
    $failed += 1
  end
end

def test_has_fragment(input, expected, desc)
  result = MPCDesignation.has_fragment?(input)
  if result == expected
    puts "  PASS: has_fragment?(\"#{input}\") -> #{result}"
    $passed += 1
  else
    puts "  FAIL: has_fragment?(\"#{input}\"): expected #{expected}, got #{result}"
    $failed += 1
  end
end

def test_get_fragment(input, expected, desc)
  begin
    result = MPCDesignation.get_fragment(input)
    if result == expected
      puts "  PASS: get_fragment(\"#{input}\") -> \"#{result}\""
      $passed += 1
    else
      puts "  FAIL: get_fragment(\"#{input}\"): expected \"#{expected}\", got \"#{result}\""
      $failed += 1
    end
  rescue StandardError => e
    puts "  FAIL: get_fragment(\"#{input}\"): error - #{e.message}"
    $failed += 1
  end
end

def test_get_parent(input, expected, desc)
  begin
    result = MPCDesignation.get_parent(input)
    if result == expected
      puts "  PASS: get_parent(\"#{input}\") -> \"#{result}\""
      $passed += 1
    else
      puts "  FAIL: get_parent(\"#{input}\"): expected \"#{expected}\", got \"#{result}\""
      $failed += 1
    end
  rescue StandardError => e
    puts "  FAIL: get_parent(\"#{input}\"): error - #{e.message}"
    $failed += 1
  end
end

def test_equal(d1, d2, expected, desc)
  result = MPCDesignation.designations_equal?(d1, d2)
  if result == expected
    puts "  PASS: designations_equal?(\"#{d1}\", \"#{d2}\") -> #{result}"
    $passed += 1
  else
    puts "  FAIL: designations_equal?(\"#{d1}\", \"#{d2}\"): expected #{expected}, got #{result}"
    $failed += 1
  end
end

puts '=== MPC Designation Helper Function Tests (Ruby) ==='
puts

# Test to_report_format
puts '--- to_report_format ---'
...
puts
puts '=================================================='
puts "Total: #{$passed + $failed}, Passed: #{$passed}, Failed: #{$failed}"
