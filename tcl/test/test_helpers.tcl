#!/usr/bin/env tclsh
#
# test_helpers.tcl - Test helper functions in MPC designation converter
#
# Tests format conversion (minimal <-> 12-char report format),
# fragment extraction, and designation comparison functions.
#

# Find and source the library
set scriptDir [file dirname [info script]]
source [file join $scriptDir .. src mpc_designation.tcl]

# Use namespace alias for convenience
namespace eval :: {
    proc toReportFormat {args} { MPCDesignation::toReportFormat {*}$args }
    proc fromReportFormat {args} { MPCDesignation::fromReportFormat {*}$args }
    proc hasFragment {args} { MPCDesignation::hasFragment {*}$args }
    proc getFragment {args} { MPCDesignation::getFragment {*}$args }
    proc getParent {args} { MPCDesignation::getParent {*}$args }
    proc designationsEqual {args} { MPCDesignation::designationsEqual {*}$args }
}
...
proc test_to_report {input expected desc} {
    global passed failed
    if {[catch {toReportFormat $input} result]} {
        puts "  FAIL: toReportFormat(\"$input\"): error - $result"
        incr failed
        return
    }
    if {$result eq $expected} {
        puts "  PASS: toReportFormat(\"$input\") -> \"$result\""
        incr passed
    } else {
        puts "  FAIL: toReportFormat(\"$input\"): expected \"$expected\", got \"$result\""
        incr failed
    }
}

proc test_from_report {input expected desc} {
    global passed failed
    if {[catch {fromReportFormat $input} result]} {
        puts "  FAIL: fromReportFormat(\"$input\"): error - $result"
        incr failed
        return
    }
    if {$result eq $expected} {
        puts "  PASS: fromReportFormat(\"$input\") -> \"$result\""
        incr passed
    } else {
        puts "  FAIL: fromReportFormat(\"$input\"): expected \"$expected\", got \"$result\""
        incr failed
    }
}

proc test_has_fragment {input expected desc} {
    global passed failed
    set result [hasFragment $input]
    if {$result == $expected} {
        puts "  PASS: hasFragment(\"$input\") -> $result"
        incr passed
    } else {
        puts "  FAIL: hasFragment(\"$input\"): expected $expected, got $result"
        incr failed
    }
}

proc test_get_fragment {input expected desc} {
    global passed failed
    if {[catch {getFragment $input} result]} {
        puts "  FAIL: getFragment(\"$input\"): error - $result"
        incr failed
        return
    }
    if {$result eq $expected} {
        puts "  PASS: getFragment(\"$input\") -> \"$result\""
        incr passed
    } else {
        puts "  FAIL: getFragment(\"$input\"): expected \"$expected\", got \"$result\""
        incr failed
    }
}

proc test_get_parent {input expected desc} {
    global passed failed
    if {[catch {getParent $input} result]} {
        puts "  FAIL: getParent(\"$input\"): error - $result"
        incr failed
        return
    }
    if {$result eq $expected} {
        puts "  PASS: getParent(\"$input\") -> \"$result\""
        incr passed
    } else {
        puts "  FAIL: getParent(\"$input\"): expected \"$expected\", got \"$result\""
        incr failed
    }
}

proc test_equal {d1 d2 expected desc} {
    global passed failed
    set result [designationsEqual $d1 $d2]
    if {$result == $expected} {
        puts "  PASS: designationsEqual(\"$d1\", \"$d2\") -> $result"
        incr passed
    } else {
        puts "  FAIL: designationsEqual(\"$d1\", \"$d2\"): expected $expected, got $result"
        incr failed
    }
}

puts "=== MPC Designation Helper Function Tests (TCL) ===\n"
