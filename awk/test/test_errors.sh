#!/bin/bash
# test_errors.sh - Test suite for MPC Designation Converter (AWK)

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
AWK_LIB="$SCRIPT_DIR/../src/mpc_designation.awk"
AWK_MAIN="$SCRIPT_DIR/../src/mpc_designation_main.awk"
...
test_convert() {
    ...
    local expected="$2"
    local result
    result=$(echo "$input" | awk -f "$AWK_LIB" -f "$AWK_MAIN")
...
echo "Running AWK MPC Designation tests..."
echo ""
