#!/usr/bin/awk -f
# test_helpers.awk - Test helper functions in MPC designation converter
#
# Usage: awk -f src/mpc_designation.awk -f test/test_helpers.awk

BEGIN {
    passed = 0
    failed = 0

    print "=== MPC Designation Helper Function Tests (AWK) ==="
    print ""
...
function test_has_fragment(input, expected, desc) {
    ...
    if (result == expected) {
        ...
        print "  FAIL: has_fragment(\"" input "\"): expected " expected ", got " result
        failed++
    }
}
...
function test_equal(d1, d2, expected, desc) {
    ...
    if (result == expected) {
        ...
        print "  FAIL: designations_equal(\"" d1 "\", \"" d2 "\"): expected " expected ", got " result
        failed++
    }
}
